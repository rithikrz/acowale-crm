...
router.post('/forms/:slug/feedback', feedbackSubmitLimiter, validate(submitFeedbackSchema), async (req, res, next) => {
  try {
    const { slug } = req.params;
    ...
    logger.info({ msg: 'Feedback submitted successfully via slug', feedbackId: feedback.id, slug });
...
router.post('/feedback', feedbackSubmitLimiter, validate(legacySubmitFeedbackSchema), async (req, res, next) => {
  try {
    ...
    logger.info({ msg: 'Legacy feedback submitted successfully', feedbackId: feedback.id, formId });
