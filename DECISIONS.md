...
- **Securing Multi-Tenant Scoping**: Enforcing scoping at the database query layer rather than the UI layer. An early challenge was preventing users from querying feedback records of other users by directly calling `/api/forms/:id/feedback` with a known ID. The solution was joining the `Form` query against `req.user.id` on _every_ request and throwing a standardized `404 Not Found` (instead of a leaky `403`) to obscure existences.
